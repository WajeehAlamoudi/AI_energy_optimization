...
def train_rl_agent(HOME_NAME="Default", NUM_EPISODES=50, MAX_STEPS_PER_EPISODE=24, SAVE_EVERY=10):
    ...
    if lstm_path.exists():
        ...
        state_size = env.state_size + 2  # 11 features + [pred_temp, pred_kWh]
    else:
    ...
    for episode in tqdm(range(1, NUM_EPISODES + 1), desc="Training Progress", ncols=100):
        ...
        for step in range(MAX_STEPS_PER_EPISODE):
            if lstm:
                ...
                lstm_input = state  # state is the 11-feature vector from env
                predicted_temp, predicted_kWh = lstm.predict(lstm_input)

                # Combine current & forecasted values
                state_input = np.concatenate([
                    state,
                    np.array([predicted_temp, predicted_kWh], dtype=np.float32)
                ])
            ...
            # Store experience
            if lstm:
                next_lstm_input = next_state
                next_pred_temp, next_pred_kWh = lstm.predict(next_lstm_input)
                next_state_input = np.concatenate([
                    next_state,
                    np.array([next_pred_temp, next_pred_kWh], dtype=np.float32)
                ])
                agent.remember(state_input, action_idx, reward, next_state_input, done)
            else:
